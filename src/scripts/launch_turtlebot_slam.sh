#!/bin/bash
echo "Starting TurtleBot3 with SLAM and RViz Visualization..."
echo "This will open 5 terminals automatically"
...
# Terminal 4: RViz with SLAM
gnome-terminal --title="RViz" -- bash -c "source ~/setup_turtlebot.sh; rviz2 -d ~/turtlebot_slam_config.rviz; exec bash"

# Terminal 5: GPS Logger
gnome-terminal --title="GPS Logger" -- bash -c "source ~/setup_turtlebot.sh; python3 auto_gps_logger.py; exec bash"

echo "All terminals launched! SLAM mapping is now active."
echo "Use 'ros2 run teleop_twist_keyboard teleop_twist_keyboard' to move robot and build map"
